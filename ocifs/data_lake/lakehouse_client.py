...
class LakehouseClient(object):
    ...
    def __init__(self, config, lake_service_api_endpoint: str = None, **kwargs):
        ...
        logger.debug(
            f"lakehouse service api endpoint is: {self.lake_service_api_endpoint}"
        )
    ...
    def get_lakehouse_mount(
        ...
        **kwargs,
        ...
        if mount_type not in mount_type_allowed_values:
            raise ValueError(
                "Invalid value for `mount_type`, must be one of {0}".format(
                    mount_type_allowed_values
                )
            )
        query_params = {}
        if mount_type == "MANAGED":
            ...
            if mount_scope_entity_type == "DATABASE" and not mount_scope_schema_key:
                raise ValueError(
                    "mount_scope_schema_key parameter cannot be None, whitespace or empty string"
                )
            if mount_scope_entity_type == "TABLE":
                if not mount_scope_schema_key or not mount_scope_table_key:
                    raise ValueError(
                        "mount_scope_schema_key and mount_scope_table_key  parameters cannot be None, "
                        "whitespace or empty string"
                    )
            if mount_scope_entity_type == "USER":
                if not mount_scope_user_id:
                    raise ValueError(
                        "mount_scope_user_id parameter cannot be None, whitespace or empty string"
                    )
            query_params = {
            ...
            query_params = {"mountType": mount_type}
        header_params = {
            "accept": "application/json",
            "content-type": "application/json",
            "opc-request-id": kwargs.get("opc_request_id", missing),
